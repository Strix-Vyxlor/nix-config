{
  ...
  cfg = config.strixos.programs.media.discord;
  styleCfg = config.strixos.style;
in {
  options.strixos.programs.media.discord = mkOption {
    type = types.bool;
    default = false;
    description = ''
      enable discord with vendord
    '';
  };
  ...
  config = mkIf cfg {
    ...
    home.file.".config/Vencord/themes/stylix.css".text = ''
      /**
      * @name base16-discord
      * @author @bypass_
      * @version 0.1.2
      * @website https://github.com/imbypass/base16-Discord
      * @description base16 theme for Discord
      **/


      :root {
          --base00: #${config.lib.stylix.colors.base00}; /* Black */
          --base01: #${config.lib.stylix.colors.base01}; /* Bright Black */
